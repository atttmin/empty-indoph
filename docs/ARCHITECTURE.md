...
| `inlineNote(for:kind:)` | 今译 / 导读 / 辩难 / 文献逐段镜片（纯文本路径；瞬态错误经 `AITransientRetry` 退避重试） |
...
[`TranslationStore`](../Empty/Services/TranslationStore.swift) 以稳定文本哈希（FNV-1a，空白归一化）为键持久化每段镜片（`ParagraphTranslation`，本地 store）。双语模式下 Mac 还会预译当前章及后两章的章节标题（`Chapter.pretranslatedAt` 标记）；所有非原文镜片在云端 provider 下都会后台预热当前章与后两章的段落缓存，翻页不等待。
...
Mac 阅读器额外功能：左右分栏今译、导读 / 辩难 / 文献镜片、结构化章节概览、☰ 目录面板、边注、思维链接、跨段选取、TTS（[`ReadingAloud`](../Empty/Services/ReadingAloud.swift)）。

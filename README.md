...
- **双语对照 / 导读**（EPUB）— Mac 为左右分栏对照（段落配对、悬停高亮），iOS 为段下对照（「译」一键开关）；**预译 + 永不阻塞**：原文先渲染、译文从本地缓存补齐，阅读时自动预译后两章，**不重复翻译**（`TranslationStore` 持久缓存，☰ 目录里可见每章预译状态与全书缓存量）
...
当前 **193/193** 单元测试全部通过；UI smoke / 截图测试在 `EmptyUITests` 中单独运行。
...
- [ ] 伴读结束时自动提炼主题、伴读镜片（今译 / 辩难 / 文献）、多条 echoes、Passkey + CloudKit 记忆同步

...
- **阅读进度** — 章内字符级（`utf16Offset`）进度与会话记录，翻页即上报
...
- **双语对照 / 导读**（Mac · EPUB）— 逐段翻译或白话导读，随阅读在原文段落下展开，按章缓存
- **问书** — 检索已读段落 + grounded 回答（带引用），答案可「存为卡片」
- **词汇释义** — 选中查词，接入间隔复习
- **思维链接**（Mac）— 跨书高亮的主题关联发现，可存为链接卡
- **伴读面板**（Mac）— 基于已读内容的对话式辅助
- **书库「上次读到」**（Mac）— 防剧透 AI 回顾 + 剩余阅读时长估计
